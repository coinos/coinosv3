...
import {
  ...
  walletForSigner, publishWalletBackup, fetchWalletBackup, nostrConnect, nostrConnectPending, resumeBunker } from '../nostr-login.js';
...
export function nostrLoginFeature(ctx) {
  ...
  async function startNostrConnect(run, resume = null) {
    if (nc) nc.cancel();
    ncRun = run;
    const token = (ncToken = {});
    let mine;
    try { mine = await nostrConnect({ resume }); } catch {
  ...
  // A reload mid-nostrconnect (the signer app's callbackUrl navigating the
  // tab home, or Android discarding the frozen tab) used to strand the
  // login: the approval lands on the relays and nobody is waiting. On boot,
  // pick the persisted handshake back up — the poller finds the stored
  // approval and the login finishes on its own. Deferred a beat so the boot
  // restore has decided whether this tab is even logged out.
  setTimeout(() => {
    const pending = !nc && !wallet.mnemonic && ui.screen === 'unlock' && nostrConnectPending();
    if (!pending) return;
    if (ui.onb && ['welcome', 'signin'].includes(ui.onb.step)) ui.onb = { step: 'signin' };
    ui.nostrLoginOpen = true;
    ui.nostrConnectOpen = true;
    startNostrConnect(loginWith, pending);
    render();
  }, 400);
